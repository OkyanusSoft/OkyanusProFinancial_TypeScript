# 📘 دليل النشر والتثبيت — نظام إدارة المطاعم
### النظام المالي المتكامل v3.0 | أوكيانوس سوفت — Okyanus Soft

دليل عملي كامل لتشغيل النظام (الواجهة الأمامية + الخادم المركزي + قاعدة MySQL) على:
- **الجزء الأول**: سيرفر محلي — Windows 11 / 10 (شبكة المطعم الداخلية)
- **الجزء الثاني**: سيرفر خارجي — استضافة cPanel (إدارة مركزية لعدة فروع)
...
## 6) ربط أجهزة المطعم (الكاشير والمحاسب والمدير)
...
- **جهاز الكاشير** → يدخل بحساب الكاشير ويعمل على نقاط البيع (نمط المطاعم والطاولات).
...
2. cPanel ← **Setup Node.js App** ← **Create Application**:
   ...
   - Application URL: `api.restaurant-domain.com` (سجل فرعي DNS من نوع A)
...
5. تحقق: `https://api.restaurant-domain.com/health`
...
1. ارفع **محتويات** مجلد `dist/` إلى `public_html/` (أو مجلد السجل الفرعي `app.restaurant-domain.com`).
...
> 💡 **ملاحظة المزامنة**: النسخة المنشورة تعمل فوراً بمحرك المزامنة المدمج (حفظ مركزي في كل جهاز + بث لحظي بين نوافذه). لربطها بالخادم المركزي عبر WebSocket الآمن، استخدم العنوان `wss://api.restaurant-domain.com/ws` في إعدادات «واجهة الربط API» داخل النظام.
...
| المتغير | مثال | الشرح |
|---|---|---|
| `VITE_API_URL` | `https://api.restaurant-domain.com` | عنوان الخادم المركزي |
| `VITE_WS_URL` | `wss://api.restaurant-domain.com/ws` | قناة البث اللحظي الآمنة |
...
- [ ] تسمية أجهزة المطعم مكتملة (كاشير/حسابات/مدير)
